...
const BlogTable = () => {
    ...
    const fetchProducts = async () => {
        ...
        try {
            ...
            const response = await axios.get("http://127.0.0.1:9000/admin/offer/", {
                headers: {
                    'Authorization': `${token}`,
                },
            });
    ...
    const handleDelete = async () => {
        try {
            const token = localStorage.getItem('token');
            await axios.delete(`http://127.0.0.1:9000/admin/Bepocart-offer-delete/${deleteProductId}/`, {
                headers: {
                    'Authorization': `${token}`,
                },
            });
    ...
    const handleStatusToggle = async (productId, currentStatus) => {
        try {
            const token = localStorage.getItem('token');
            const response = await axios.put(`http://127.0.0.1:9000/admin/Bepocart-offer/${productId}/toggle-status/`, {}, {
                headers: {
                    'Authorization': `${token}`,
                },
            });
