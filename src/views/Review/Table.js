...
const TableBanner = () => {
    ...
    const fetchProducts = async () => {
        ...
        try {
            ...
            const response = await axios.get("http://127.0.0.1:9000/admin/Bepocart-Product-Review/", {
                headers: {
                    'Authorization': `${token}`,
                },
            });
    ...
    const handleApprove = async (id) => {
        try {
            const token = localStorage.getItem('token');
            const response = await axios.put(`http://127.0.0.1:9000/admin/Bepocart-approve-review/${id}/`, null, {
                headers: {
                    'Authorization': `${token}`,
                },
            });
                            ...
                            <TableRow key={product.id}>
                                ...
                                <TableCell align="center">
                                    <Link to={`/user-coin-data/${product.product}/`} style={{ textDecoration: 'none', color: 'inherit' }}>
                                        <img
                                            src={`http://127.0.0.1:9000/${product.product_image}`}
                                            alt={product.product}
                                            style={{ maxWidth: "70px", maxHeight: "70px" }}
                                        />
                                    </Link>
