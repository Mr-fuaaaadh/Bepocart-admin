...
const TableBanner = ({ searchQuery }) => {
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                ...
                const response = await axios.get("http://127.0.0.1:9000/admin/Bepocart-Orders/", {
                    headers: {
                        'Authorization': `${token}`,
                    },
                });
    ...
    const handleStatusChange = async (productId, newStatus) => {
        try {
            const token = localStorage.getItem('token');
            await axios.put(`http://127.0.0.1:9000/admin/Bepocart-Order-status-update/${productId}/`,
                { status: newStatus },
                {
                    headers: {
                        'Authorization': `${token}`,
                        ...
                        {filteredProducts.map((product) => (
                            <TableRow key={product.id}>
                                ...
                                <TableCell>
                                    <Link to={`/product-image-form/${product.id}/`} style={{ textDecoration: 'none', color: 'inherit' }}>
                                        <img
                                            src={`http://127.0.0.1:9000/${product.customerImage}`}
